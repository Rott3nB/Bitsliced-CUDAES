#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <time.h>
#include <math.h>
#include <inttypes.h>

#include "cuda_runtime.h"
#include "device_launch_parameters.h"
#include "cuda_runtime_api.h"







uint8_t ctx_key[32];
uint8_t ctx_enckey[32];
uint8_t ctx_deckey[32];


#define AES_BLOCK_SIZE 16
#define THREADS_PER_BLOCK 1024


#define F(x)   (((x)<<1) ^ ((((x)>>7) & 1) * 0x1b))
#define FD(x)  (((x) >> 1) ^ (((x) & 1) ? 0x8d : 0))


#define gpuErrchk(ans) { gpuAssert((ans), __FILE__, __LINE__); }
inline void gpuAssert(cudaError_t code, const char* file, int line, bool abort = true)
{
    if (code != cudaSuccess)
    {
        fprintf(stderr, "GPUassert: %s %s %d\n", cudaGetErrorString(code), file, line);
        if (abort) exit(code);
    }
}

// S table
__constant__ static const uint8_t sbox[256] = {
    
};


// inv S table
__constant__ static const uint8_t sboxinv[256] = {
    
};


// x-time operation
__device__ uint8_t rj_xtime(uint8_t x) {
    return (x & 0x80) ? ((x << 1) ^ 0x1b) : (x << 1);
}


// subbyte operation
__device__ void aes_subBytes(uint8_t* buf) {
    register uint8_t i, b;
    for (i = 0; i < 16; ++i) {
        b = buf[i];
        buf[i] = sbox[b];
    }
}


// inv subbyte operation
__device__ void aes_subBytes_inv(uint8_t* buf) {
    register uint8_t i, b;
    for (i = 0; i < 16; ++i) {
        b = buf[i];
        buf[i] = sboxinv[b];
    }
}


// add round key operation
__device__ void aes_addRoundKey(uint8_t* buf, uint8_t* key) {
    register uint8_t i = 16;
    while (i--) {
        buf[i] ^= key[i];
    }
}


// add round key at beginning
__device__ void aes_addRoundKey_cpy(uint8_t* buf, uint8_t* key, uint8_t* cpk) {
    register uint8_t i = 16;
    while (i--) {
        buf[i] ^= (cpk[i] = key[i]);
        cpk[16 + i] = key[16 + i];
    }
}



// shift row operation
__device__ void aes_shiftRows(uint8_t* buf) {
    register uint8_t i, j;
    i = buf[1];
    buf[1] = buf[5];
    buf[5] = buf[9];
    buf[9] = buf[13];
    buf[13] = i;
    i = buf[10];
    buf[10] = buf[2];
    buf[2] = i;
    j = buf[3];
    buf[3] = buf[15];
    buf[15] = buf[11];
    buf[11] = buf[7];
    buf[7] = j;
    j = buf[14];
    buf[14] = buf[6];
    buf[6] = j;
}



// inv shift row operation
__device__ void aes_shiftRows_inv(uint8_t* buf) {
    register uint8_t i, j;
    i = buf[1];
    buf[1] = buf[13];
    buf[13] = buf[9];
    buf[9] = buf[5];
    buf[5] = i;
    i = buf[2];
    buf[2] = buf[10];
    buf[10] = i;
    j = buf[3];
    buf[3] = buf[7];
    buf[7] = buf[11];
    buf[11] = buf[15];
    buf[15] = j;
    j = buf[6];
    buf[6] = buf[14];
    buf[14] = j;
}


// mix column operation
__device__ void aes_mixColumns(uint8_t* buf) {
    register uint8_t i, a, b, c, d, e;
    for (i = 0; i < 16; i += 4) {
        a = buf[i];
        b = buf[i + 1];
        c = buf[i + 2];
        d = buf[i + 3];
        e = a ^ b ^ c ^ d;
        buf[i] ^= e ^ rj_xtime(a ^ b);
        buf[i + 1] ^= e ^ rj_xtime(b ^ c);
        buf[i + 2] ^= e ^ rj_xtime(c ^ d);
        buf[i + 3] ^= e ^ rj_xtime(d ^ a);
    }
}


// inv mix column operation
__device__ void aes_mixColumns_inv(uint8_t* buf) {
    register uint8_t i, a, b, c, d, e, x, y, z;
    for (i = 0; i < 16; i += 4) {
        a = buf[i];
        b = buf[i + 1];
        c = buf[i + 2];
        d = buf[i + 3];
        e = a ^ b ^ c ^ d;
        z = rj_xtime(e);
        x = e ^ rj_xtime(rj_xtime(z ^ a ^ c));
        y = e ^ rj_xtime(rj_xtime(z ^ b ^ d));
        buf[i] ^= x ^ rj_xtime(a ^ b);
        buf[i + 1] ^= y ^ rj_xtime(b ^ c);
        buf[i + 2] ^= x ^ rj_xtime(c ^ d);
        buf[i + 3] ^= y ^ rj_xtime(d ^ a);
    }
}


// add expand key operation
__device__ __host__ void aes_expandEncKey(uint8_t* k, uint8_t* rc, const uint8_t* sb) {
    register uint8_t i;

    k[0] ^= sb[k[29]] ^ (*rc);
    k[1] ^= sb[k[30]];
    k[2] ^= sb[k[31]];
    k[3] ^= sb[k[28]];
    *rc = F(*rc);

    for (i = 4; i < 16; i += 4) {
        k[i] ^= k[i - 4];
        k[i + 1] ^= k[i - 3];
        k[i + 2] ^= k[i - 2];
        k[i + 3] ^= k[i - 1];
    }

    k[16] ^= sb[k[12]];
    k[17] ^= sb[k[13]];
    k[18] ^= sb[k[14]];
    k[19] ^= sb[k[15]];

    for (i = 20; i < 32; i += 4) {
        k[i] ^= k[i - 4];
        k[i + 1] ^= k[i - 3];
        k[i + 2] ^= k[i - 2];
        k[i + 3] ^= k[i - 1];
    }

}



// inv add expand key operation
__device__ void aes_expandDecKey(uint8_t* k, uint8_t* rc) {
    uint8_t i;

    for (i = 28; i > 16; i -= 4) {
        k[i + 0] ^= k[i - 4];
        k[i + 1] ^= k[i - 3];
        k[i + 2] ^= k[i - 2];
        k[i + 3] ^= k[i - 1];
    }

    k[16] ^= sbox[k[12]];
    k[17] ^= sbox[k[13]];
    k[18] ^= sbox[k[14]];
    k[19] ^= sbox[k[15]];

    for (i = 12; i > 0; i -= 4) {
        k[i + 0] ^= k[i - 4];
        k[i + 1] ^= k[i - 3];
        k[i + 2] ^= k[i - 2];
        k[i + 3] ^= k[i - 1];
    }

    *rc = FD(*rc);
    k[0] ^= sbox[k[29]] ^ (*rc);
    k[1] ^= sbox[k[30]];
    k[2] ^= sbox[k[31]];
    k[3] ^= sbox[k[28]];
}


// key initition
void aes256_init(uint8_t* k) {
    uint8_t rcon = 1;
    register uint8_t i;

    for (i = 0; i < sizeof(ctx_key); i++) {
        ctx_enckey[i] = ctx_deckey[i] = k[i];
    }
    for (i = 8; --i;) {
        aes_expandEncKey(ctx_deckey, &rcon, sbox);
    }
}



// aes encrypt algorithm one thread/one block with AES_BLOCK_SIZE 
__global__ void aes256_encrypt_ecb(uint8_t* buf_d, unsigned long numbytes, uint8_t* ctx_enckey_d, uint8_t* ctx_key_d) {
    uint8_t i, rcon;
    uint8_t buf_t[AES_BLOCK_SIZE]; // thread buffer
    unsigned long offset = (blockIdx.x * THREADS_PER_BLOCK * AES_BLOCK_SIZE) + (threadIdx.x * AES_BLOCK_SIZE);
    if (offset >= numbytes) { return; }

    unsigned long key_offset = offset * 2;
    //printf("Thread %d\n", threadIdx.x);


    /*
     *    if (threadIdx.x == 0) {
        for (int i = 0; i < 32; i++) printf("%02x", ctx_key_d[i]);
    }
     */



    memcpy(buf_t, &buf_d[offset], AES_BLOCK_SIZE);
    aes_addRoundKey_cpy(buf_t, ctx_enckey_d, &ctx_key_d[key_offset]);
    for (i = 1, rcon = 1; i < 14; ++i) {
        aes_subBytes(buf_t);
        aes_shiftRows(buf_t);
        aes_mixColumns(buf_t);
        if (i & 1) {
            aes_addRoundKey(buf_t, &ctx_key_d[16 + key_offset]);
        }
        else {
            aes_expandEncKey(&ctx_key_d[key_offset], &rcon, sbox), aes_addRoundKey(buf_t, &ctx_key_d[key_offset]);
        }
    }
    aes_subBytes(buf_t);
    aes_shiftRows(buf_t);
    aes_expandEncKey(&ctx_key_d[key_offset], &rcon, sbox);
    aes_addRoundKey(buf_t, &ctx_key_d[key_offset]);
    /* copy thread buffer back into global memory */
    //printf("Thread %d\n", threadIdx.x);
    memcpy(&buf_d[offset], buf_t, AES_BLOCK_SIZE);
}



// aes decrypt algorithm
__global__ void aes256_decrypt_ecb(uint8_t* buf_d, unsigned long numbytes, uint8_t* ctx_deckey_d, uint8_t* ctx_key_d) {
    uint8_t i, rcon;
    uint8_t buf_t[AES_BLOCK_SIZE];
    unsigned long offset = (blockIdx.x * THREADS_PER_BLOCK * AES_BLOCK_SIZE) + (threadIdx.x * AES_BLOCK_SIZE);
    if (offset >= numbytes) { return; }

    memcpy(buf_t, &buf_d[offset], AES_BLOCK_SIZE);
    aes_addRoundKey_cpy(buf_t, ctx_deckey_d, &ctx_key_d[offset * 2]);
    aes_shiftRows_inv(buf_t);
    aes_subBytes_inv(buf_t);
    for (i = 14, rcon = 0x80; --i;) {
        if ((i & 1)) {
            aes_expandDecKey(&ctx_key_d[offset * 2], &rcon);
            aes_addRoundKey(buf_t, &ctx_key_d[16 + offset * 2]);
        }
        else {
            aes_addRoundKey(buf_t, &ctx_key_d[offset * 2]);
        }
        aes_mixColumns_inv(buf_t);
        aes_shiftRows_inv(buf_t);
        aes_subBytes_inv(buf_t);
    }
    aes_addRoundKey(buf_t, &ctx_key_d[offset * 2]);
    /* copy thread back into global memory */
    memcpy(&buf_d[offset], buf_t, AES_BLOCK_SIZE);
    __syncthreads();
}



// aes encrypt demo
void encryptdemo(uint8_t key[32], uint8_t* buf, unsigned long numbytes, uint8_t* ctx_key_d) {
    uint8_t* buf_d;
    uint8_t* ctx_enckey_d;


    cudaEvent_t start, stop;
    float time_h_to_d_buf = 0.0;
    float time_h_to_d_key = 0.0;
    float time_k = 0.0;
    float time_d_to_h_buf = 0.0;
    float time_d_to_h_key = 0.0;
    float elapsed_time;


    dim3 dimBlock(ceil((double)numbytes / (double)(THREADS_PER_BLOCK * AES_BLOCK_SIZE)));
    dim3 dimGrid(THREADS_PER_BLOCK);


    cudaMemcpyToSymbol(sbox, sbox, sizeof(uint8_t) * 256);

    printf("\nBeginning encryption\n");
    aes256_init(key);

    gpuErrchk(cudaMallocManaged((void**)&buf_d, numbytes));
    gpuErrchk(cudaMallocManaged((void**)&ctx_enckey_d, sizeof(ctx_enckey)));

    //for (int a = 0; a < (ceil((double)numbytes / (double)(THREADS_PER_BLOCK * AES_BLOCK_SIZE))); a++) {
    //    for (int i = 0; i < 32; i++) printf("%02x", ctx_key_d[a * 32 + i]);
    //    printf("\n");
    //}

    // Recording host-to-device
    cudaEventCreate(&start); // create event
    cudaEventCreate(&stop);

    cudaEventRecord(start, 0); // start record
    cudaMemcpy(buf_d, buf, numbytes, cudaMemcpyHostToDevice); // 1. copy buf from host to device
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_h_to_d_buf, start, stop); // calculate delta time





    // Recording host to device key
    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(ctx_enckey_d, ctx_enckey, sizeof(ctx_enckey), cudaMemcpyHostToDevice)); // 2. copy key from host to device
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_h_to_d_key, start, stop); // calculate delta time




    printf("Creating %d threads over %d blocks\n", dimBlock.x * dimGrid.x, dimBlock.x);


    uint8_t* buf_x = (uint8_t*)malloc(32);
    cudaMemcpy(buf_x, ctx_enckey_d, sizeof(ctx_key), cudaMemcpyDeviceToHost);
    //for (int i = 0; i < 32; i++) printf("%02x", buf_x[i]);



    // Recording kernel
    cudaEventRecord(start, 0); // start record
    aes256_encrypt_ecb << <dimBlock, dimGrid >> > (buf_d, numbytes, ctx_enckey_d, ctx_key_d); // 3. kernel
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_k, start, stop); // calculate delta time

    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());


    // Record device to host buf
    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(buf, buf_d, numbytes, cudaMemcpyDeviceToHost)); // 4. copy buf from device to host
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_d_to_h_buf, start, stop); // calculate delta time



    // Record device to host key
    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(ctx_enckey, ctx_enckey_d, sizeof(ctx_enckey), cudaMemcpyDeviceToHost)); // 5. copy key from device to host
    cudaMemcpy(ctx_key, ctx_key_d, sizeof(ctx_key), cudaMemcpyDeviceToHost);
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_d_to_h_key, start, stop); // calculate delta time




    // printf
    printf("Time used memcpy from host to device buf %0.3g seconds\n", time_h_to_d_buf / 1000);
    printf("Time used memcpy from host to device key %0.3g seconds\n", time_h_to_d_key / 1000);
    printf("Time used memcpy kernel %0.3g seconds\n", time_k / 1000);
    printf("Time used memcpy from device to host buf %0.3g seconds\n", time_d_to_h_buf / 1000);
    printf("Time used memcpy from device to host key %0.3g seconds\n", time_d_to_h_key / 1000);
    printf("GPU encryption throughput: %f bytes/second\n", (double)(numbytes) / ((double)(time_k) / CLOCKS_PER_SEC));



    cudaFree(buf_d);
    cudaFree(ctx_enckey_d);

}









// aes decrypt demo
void decryptdemo(uint8_t key[32], uint8_t* buf, unsigned long numbytes, uint8_t* ctx_key_d) {
    uint8_t* buf_d;
    uint8_t* ctx_deckey_d;

    dim3 dimBlock(ceil((double)numbytes / (double)(THREADS_PER_BLOCK * AES_BLOCK_SIZE)));
    dim3 dimGrid(THREADS_PER_BLOCK);

    cudaEvent_t start, stop;
    float time_h_to_d_buf = 0.0;
    float time_h_to_d_key = 0.0;
    float time_k = 0.0;
    float time_d_to_h_buf = 0.0;
    float time_d_to_h_key = 0.0;
    float elapsed_time;


    cudaMemcpyToSymbol(sboxinv, sboxinv, sizeof(uint8_t) * 256);

    printf("\nBeginning decryption\n");

    gpuErrchk(cudaMallocManaged((void**)&buf_d, numbytes));
    gpuErrchk(cudaMallocManaged((void**)&ctx_deckey_d, sizeof(ctx_deckey)));

    // Recording host-to-device
    cudaEventCreate(&start); // create event
    cudaEventCreate(&stop);

    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(buf_d, buf, numbytes, cudaMemcpyHostToDevice)); // 1/ copy buf from host to device
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_h_to_d_buf, start, stop); // calculate delta time






    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(ctx_deckey_d, ctx_deckey, sizeof(ctx_deckey), cudaMemcpyHostToDevice)); // 2. copy key from host to device  
    //cudaMemcpy(ctx_key_d, ctx_key, sizeof(ctx_key), cudaMemcpyHostToDevice);
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_h_to_d_key, start, stop); // calculate delta time


    printf("Creating %d threads over %d blocks\n", dimBlock.x * dimGrid.x, dimBlock.x);



    cudaEventRecord(start, 0); // start record
    aes256_decrypt_ecb << <dimBlock, dimGrid >> > (buf_d, numbytes, ctx_deckey_d, ctx_key_d);  // 3. kernel
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_k, start, stop); // calculate delta time





    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(buf, buf_d, numbytes, cudaMemcpyDeviceToHost));      // 4. copy buf from device to host
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_d_to_h_buf, start, stop); // calculate delta time




    cudaEventRecord(start, 0); // start record
    gpuErrchk(cudaMemcpy(ctx_deckey, ctx_deckey_d, sizeof(ctx_deckey), cudaMemcpyDeviceToHost));
    cudaMemcpy(ctx_key, ctx_key_d, sizeof(ctx_key), cudaMemcpyDeviceToHost);
    cudaEventRecord(stop, 0); // end record
    cudaEventSynchronize(start); // waits for an event to complete
    cudaEventSynchronize(stop);    //Waits for an event to complete
    cudaEventElapsedTime(&time_d_to_h_key, start, stop); // calculate delta time




    // printf
    printf("De-Time used memcpy from host to device buf %0.3g seconds\n", time_h_to_d_buf / 1000);
    printf("De-Time used memcpy from host to device key %0.3g seconds\n", time_h_to_d_key / 1000);
    printf("De-Time used memcpy kernel %0.3g seconds\n", time_k / 1000);
    printf("De-Time used memcpy from device to host buf %0.3g seconds\n", time_d_to_h_buf / 1000);
    printf("De-Time used memcpy from device to host key %0.3g seconds\n", time_d_to_h_key / 1000);
    printf("GPU encryption throughput: %f bytes/second\n", (double)(numbytes) / ((double)(time_k) / CLOCKS_PER_SEC));



    cudaFree(buf_d);
    cudaFree(ctx_deckey_d);

}


__global__ void GPU_init() { }



int main() {

    // open file
    FILE* file;
    uint8_t* buf;
    unsigned long numbytes;
    char* fname;
    int mili_sec, i;
    int padding;
    uint8_t* ctx_key_d;
    uint8_t key[32];

    int deviceCount = 0;
    cudaError_t error_id = cudaGetDeviceCount(&deviceCount);

    if (error_id != cudaSuccess) {
        printf("Error: %s\n", cudaGetErrorString(error_id));
        printf("Exiting...\n");
        exit(EXIT_FAILURE);
    }

    if (deviceCount == 0) {
        printf("There are no available device(s) that support CUDA\n");
        exit(EXIT_FAILURE);
    }


    // handle txt file
    fname = "C:\\Users\\Tim\\Downloads\\AES-ON-GPU-maste22r\\AES-ON-GPU-master\\Dataset_AES\\input.txt";
    file = fopen(fname, "r");
    if (file == NULL) { printf("File %s doesn't exist\n", fname); exit(1); }
    printf("Opened file %s\n", fname);
    fseek(file, 0L, SEEK_END);
    numbytes = ftell(file);
    padding = (AES_BLOCK_SIZE - numbytes % AES_BLOCK_SIZE) % AES_BLOCK_SIZE;
    printf("Size is %lu\n", numbytes);

    // copy file into memory
    fseek(file, 0L, SEEK_SET);
    buf = (uint8_t*)calloc(numbytes + padding, sizeof(uint8_t));
    if (buf == NULL) exit(1);
    if (fread(buf, 1, numbytes, file) != numbytes)
    {
        printf("Unable to read all bytes from file %s\n", fname);
        exit(EXIT_FAILURE);
    }
    fclose(file);

    // calculate the padding
    numbytes += padding;
    printf("Padding file with %d bytes for a new size of %lu\n", padding, numbytes);

    // generate key
    for (i = 0; i < sizeof(key); i++) key[i] = i;

    // this is to force nvcc to put the gpu initialization here
    GPU_init << <1, 1 >> > ();

    // encryption


    //for (i = 0; i < sizeof(key); i++) printf("%02x", key[i]);
    //printf("\n");



    gpuErrchk(cudaMallocManaged((void**)&ctx_key_d, ceil((double)numbytes / (double)(THREADS_PER_BLOCK * AES_BLOCK_SIZE)) * THREADS_PER_BLOCK * 32));

    encryptdemo(key, buf, numbytes, ctx_key_d);

    cudaFree(ctx_key_d);
    //for (i = 0; i < numbytes; i++) printf("%02x", buf[i]);
    //printf("\n");
    gpuErrchk(cudaMallocManaged((void**)&ctx_key_d, ceil((double)numbytes / (double)(THREADS_PER_BLOCK * AES_BLOCK_SIZE)) * THREADS_PER_BLOCK * 32));
    //for (i = 0; i < numbytes; i++) printf("%02x", buf[i]);
    //printf("\n");
    // write into file
    file = fopen("C:\\Users\\Tim\\Downloads\\AES-ON-GPU-maste22r\\AES-ON-GPU-master\\Dataset_AES\\cipher.txt", "w");
    fwrite(buf, 1, numbytes, file);
    fclose(file);

    // decryption

    decryptdemo(key, buf, numbytes, ctx_key_d);


    // write into file
    file = fopen("C:\\Users\\Tim\\Downloads\\AES-ON-GPU-maste22r\\AES-ON-GPU-master\\Dataset_AES\\output.txt", "w");
    fwrite(buf, 1, numbytes - padding, file);
    fclose(file);
    //cudaFree(ctx_key_d);
    free(buf);
    return EXIT_SUCCESS;
}